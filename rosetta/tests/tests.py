...
class LibreTranslateTestCase(TestCase):
    """Tests for LibreTranslate integration"""
    ...
    def test_libretranslate_translation_success(self, mock_post):
        """Test successful LibreTranslate translation"""
        ...
        result = translate_by_libretranslate(
            "Hello world", "en", "es", "http://localhost:5000/translate"
        )
    ...
    @mock.patch('requests.post')
    def test_libretranslate_translation_with_api_key(self, mock_post):
        """Test LibreTranslate translation with API key"""
        from rosetta.translate_utils import translate_by_libretranslate
        ...
        result = translate_by_libretranslate(
            "Hello world", "en", "fr", "http://localhost:5000/translate", "test-api-key"
        )
    ...
    @mock.patch('requests.post')
    def test_libretranslate_translation_error_handling(self, mock_post):
        """Test LibreTranslate error handling"""
        from rosetta.translate_utils import translate_by_libretranslate, TranslationException
        
        # Mock error response
        mock_response = mock.Mock()
        mock_response.status_code = 400
        mock_response.json.return_value = {"error": "Language pair not supported"}
        mock_post.return_value = mock_response
        
        with self.assertRaises(TranslationException) as context:
            translate_by_libretranslate(
                "Hello world", "en", "xx", "http://localhost:5000/translate"
            )
        
        self.assertIn("Language pair not supported", str(context.exception))
    
    @mock.patch('requests.post')
    def test_libretranslate_connection_error(self, mock_post):
        """Test LibreTranslate connection error handling"""
        from rosetta.translate_utils import translate_by_libretranslate, TranslationException
        import requests
        ...
        self.assertIn("Could not connect to LibreTranslate server", str(context.exception))
    
    @override_settings(
        LIBRETRANSLATE_URL="http://localhost:5000",  # URL without /translate endpoint
        ROSETTA_ENABLE_TRANSLATION_SUGGESTIONS=True
    )
    @mock.patch('requests.post')
    def test_libretranslate_url_normalization(self, mock_post):
        """Test that /translate is automatically appended to URL"""
        from rosetta.translate_utils import translate_by_libretranslate
        ...
        translate_by_libretranslate(
            "Hello world", "en", "es", "http://localhost:5000"
        )
        
        # Verify that the URL was normalized to include /translate
        mock_post.assert_called_once()
        call_args = mock_post.call_args
        self.assertEqual(call_args[0][0], "http://localhost:5000/translate")
    
    @override_settings(
        LIBRETRANSLATE_URL="http://localhost:5000/translate",
        ROSETTA_ENABLE_TRANSLATION_SUGGESTIONS=True
    )
    @mock.patch('rosetta.translate_utils.translate_by_libretranslate')
    def test_translate_text_view_with_libretranslate(self, mock_translate):
        """Test the translate_text view with LibreTranslate provider"""
        mock_translate.return_value = "Hola mundo"
        
        response = self.client.get(reverse('rosetta.translate_text'), {
            'from': 'en',
            'to': 'es',
            'text': 'Hello world',
        ...
        self.assertEqual(data['translation'], 'Hola mundo')
        
        mock_translate.assert_called_once_with(
            'Hello world', 'en', 'es',
            'http://localhost:5000/translate', None
        )
