...
def translate(text, from_language, to_language):
    ...
    LIBRETRANSLATE_API_KEY = getattr(settings, "LIBRETRANSLATE_API_KEY", None)

    if LIBRETRANSLATE_URL:
        return translate_by_libretranslate(
            text, from_language, to_language, LIBRETRANSLATE_URL, LIBRETRANSLATE_API_KEY
        )
...
def translate_by_libretranslate(text, from_language, to_language, api_url, api_key=None):
    """
    Translate text using LibreTranslate API.
    
    :param text: The text to translate
    :param from_language: Source language code (e.g., 'en', 'fr')
    :param to_language: Target language code (e.g., 'es', 'de')
    :param api_url: LibreTranslate API URL (e.g., 'http://localhost:5000/translate')
    ...
    data = {
        'q': text,
        'source': from_language.split('-')[0],  # Use only language code, not locale
        'target': to_language.split('-')[0],    # Use only language code, not locale
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'translatedText' in result:
                ...
                raise TranslationException("LibreTranslate returned unexpected response format")
        elif response.status_code == 403:
            raise TranslationException("LibreTranslate API key is invalid or missing")
        elif response.status_code == 400:
            try:
                error_detail = response.json().get('error', 'Bad request')
            except:
                error_detail = 'Bad request'
            raise TranslationException(f"LibreTranslate API error: {error_detail}")
        else:
            raise TranslationException(f"LibreTranslate API returned status code {response.status_code}")
            
    except requests.exceptions.ConnectionError:
        raise TranslationException("Could not connect to LibreTranslate server. Please check the URL and ensure the server is running.")
    except requests.exceptions.Timeout:
        raise TranslationException("LibreTranslate request timed out")
    except requests.exceptions.RequestException as e:
        raise TranslationException(f"LibreTranslate request error: {str(e)}")
    except json.JSONDecodeError:
        raise TranslationException("LibreTranslate returned invalid JSON response")
