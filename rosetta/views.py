    ...
    * query: a search string, where only matches are shown. Fields that are
    ...
    def get_context_data(self, **kwargs):
        ...
        rosetta_settings_js = {
            ...
            "LIBRETRANSLATE_URL": rosetta_settings.LIBRETRANSLATE_URL,
            ...
            "server_auth_key": bool(
                ...
                or rosetta_settings.LIBRETRANSLATE_URL
            ),
        }
...
def translate_text(request):
    ...
    if language_from == language_to:
        ...
        try:
            # Force LibreTranslate if requested and available
            if provider == 'libretranslate' and rosetta_settings.LIBRETRANSLATE_URL:
                from .translate_utils import translate_by_libretranslate
                translated_text = translate_by_libretranslate(
                    text, 
                    language_from, 
                    language_to, 
                    rosetta_settings.LIBRETRANSLATE_URL,
                    rosetta_settings.LIBRETRANSLATE_API_KEY
                )
            else:
